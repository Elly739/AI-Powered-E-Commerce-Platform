  ...
  return (
    <section className="auth-layout">
      ...
      <form className="auth-card" onSubmit={handleSubmit}>
        ...
        <Button type="submit" loading={submitting} trailingIcon={<ArrowRight size={16} />}>{isLogin ? 'Sign in' : 'Create account'}</Button>
...
function CartPage() {
  ...
  if (!token) return <section className="cart-page container-main"><span className="eyebrow">Your cart</span><h2>A little room<br /><em>for good things.</em></h2><p className="muted">Sign in to keep your cart with you wherever you go.</p><Link to="/login" className="btn-primary">Sign in to continue <ArrowRight size={16} /></Link></section>
...
function WishlistPage() {
  ...
  if (!token) return <section className="wishlist-page container-main"><span className="eyebrow">Your wishlist</span><h2>Keep the<br /><em>good ones.</em></h2><p className="muted">Sign in to save pieces you want to return to.</p><Link to="/login" className="btn-primary">Sign in to continue <ArrowRight size={16} /></Link></section>
...
function AdminPage() {
  ...
  return <section className="admin-page container-main"><div className="admin-heading"><div><span className="eyebrow">Admin studio</span><h2>Shape the<br /><em>next edit.</em></h2></div><span className="cart-summary">{products.length} live products</span></div><div className="admin-layout"><form className="admin-form" onSubmit={createProduct}><h3>Add a product</h3><label>Name<input value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} required /></label><label>Slug<input value={form.slug} onChange={(event) => setForm({ ...form, slug: event.target.value })} placeholder="unique-product-slug" required /></label><label>Description<textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /></label><div className="admin-form-row"><label>Price<input type="number" min="0" step="0.01" value={form.price} onChange={(event) => setForm({ ...form, price: event.target.value })} required /></label><label>Stock<input type="number" min="0" value={form.stockQuantity} onChange={(event) => setForm({ ...form, stockQuantity: event.target.value })} required /></label></div><button className="btn-primary" type="submit">Publish product <ArrowRight size={16} /></button>{message && <p className="muted">{message}</p>}</form><div className="admin-list"><span className="eyebrow">Live catalog</span>{products.map((product) => <div className="admin-product" key={product.id}><div><strong>{product.name}</strong><span>{product.categoryName || 'Uncategorized'}</span></div><span>${product.price} / {product.stockQuantity} in stock</span></div>)}</div></div></section>
}
...
function SiteHeader({ user }) {
  ...
  return <header className="site-header"><nav className="container-main nav-bar"><button className="menu-toggle" onClick={() => setMenuOpen(!menuOpen)} aria-label={menuOpen ? 'Close navigation' : 'Open navigation'}>{menuOpen ? <X size={20} /> : <Menu size={20} />}</button><Link to="/" className="brand"><span className="brand-mark">A</span> Aster & Co.</Link><form className="global-search" onSubmit={submitSearch}><Search size={16} /><input aria-label="Search products" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search the edit" /></form><div className={`nav-links ${menuOpen ? 'nav-links-open' : ''}`}><Link to="/products" onClick={() => setMenuOpen(false)}>Discover</Link><Link to="/assistant" className="assistant-link" onClick={() => setMenuOpen(false)}><Sparkles size={14} /> Ask the edit</Link><Link to="/wishlist" className="saved-link" onClick={() => setMenuOpen(false)}><Heart size={14} /> Saved</Link><Link to="/cart" onClick={() => setMenuOpen(false)}><ShoppingBag size={14} /> Cart <span className="cart-count">0</span></Link>{user ? <><Link to="/admin" className={user.role === 'admin' ? '' : 'hidden-link'} onClick={() => setMenuOpen(false)}><LayoutDashboard size={14} /> Studio</Link><Link to="/dashboard" className="profile-link" onClick={() => setMenuOpen(false)} aria-label="Open dashboard"><User size={16} /></Link></> : <Link to="/login" className="nav-cta" onClick={() => setMenuOpen(false)}>Sign in</Link>}</div></nav>{location.pathname === '/' && <div className="mobile-search container-main"><form className="global-search" onSubmit={submitSearch}><Search size={16} /><input aria-label="Search products" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search products" /></form></div>}</header>
}
  ...
  return (
    <div className="home-page">
      <section className="hero container-main">
        <div className="hero-copy"><span className="eyebrow">A considered edit for everyday life</span><h2>Find things that feel like <em>you.</em></h2><p>Discover useful, beautiful pieces selected around your rhythm, your home, and the details you notice.</p><div className="hero-actions"><Link to="/register" className="btn-primary">Start exploring <ArrowRight size={16} /></Link><Link to="/products" className="text-link">Browse the edit</Link></div></div>
